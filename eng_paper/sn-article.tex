...
\title{Reconstruction of hand trajectory with video}
...
\abstract{In this paper, we consider the problem of forecasting a time series with a complex structure. 
The complex structure means the presence of non-linear dependencies and a varying period. 
We must to find causal relationships between time series. 
...
To demonstrate the results of the work we solve the problem of hand trajectory reconstruction with video.}
...
One of the challenges of the task is to detect relationships between time series and exclude unrelated time series from the predictive model.
Solving this problem improves its quality.
...
In the work \cite{alexandrov2005automatic} automatic selection of a pair of main components is used.
The main idea is to compare the spectral densities of the principal components. 
A simple iteration over the main components \cite{usmanova2019dependencies} is also used.
...
The theoretical part of the paper shows how the Sugihara method can be applied to reduce the dimension of the trajectory space and how to combine the ideas of the PLS and CCM methods.
